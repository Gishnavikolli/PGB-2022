...
function Python2() {
  ...
  const NextLevel=()=>window.location="/Python3Quiz";
  const SameLevel=()=>window.history.go("/Python2Quiz");
  return questions.length > 0 ? (
    ...
    >
      {currentIndex >= questions.length ? (
        ...
        <Link to="/Python2Quiz"><button style={{marginLeft:"5%",backgroundColor:"white",width:"200px",padding:"8px",fontWeight:"bolder",borderRadius:"10px"}} type= "button" className='btn-next' onClick={SameLevel}>Try Again</button></Link>
        </>
        }
        </>
      ) : (
